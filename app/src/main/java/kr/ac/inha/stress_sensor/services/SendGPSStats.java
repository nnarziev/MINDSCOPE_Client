        ...
        if (intent != null) {
            ...
            try {
                if (readLocations(time_start, time_end) > 1) {
                    ...
                    int dataSourceId = prefs.getInt("TOTAL_DIST_COVERED", -1);
                    assert dataSourceId != -1;
                    DbMgr.saveMixedData(dataSourceId, time_start, 1.0f, time_start, time_end, getTotalDisCovered());

                    dataSourceId = prefs.getInt("MAX_DIST_FROM_HOME", -1);
                    assert dataSourceId != -1;
                    DbMgr.saveMixedData(dataSourceId, time_start, 1.0f, time_start, time_end, getMaxDistFromHome());

                    dataSourceId = prefs.getInt("MAX_DIST_TWO_LOCATIONS", -1);
                    assert dataSourceId != -1;
                    DbMgr.saveMixedData(dataSourceId, time_start, 1.0f, time_start, time_end, getMaxDistBtwTwoLocations());

                    dataSourceId = prefs.getInt("RADIUS_OF_GYRATION", -1);
                    assert dataSourceId != -1;
                    DbMgr.saveMixedData(dataSourceId, time_start, 1.0f, time_start, time_end, getRadiusOfGyration());

                    dataSourceId = prefs.getInt("STDDEV_OF_DISPLACEMENT", -1);
                    assert dataSourceId != -1;
                    DbMgr.saveMixedData(dataSourceId, time_start, 1.0f, time_start, time_end, getStdDevOfDisplacement());

                    dataSourceId = prefs.getInt("NUM_OF_DIF_PLACES", -1);
                    assert dataSourceId != -1;
